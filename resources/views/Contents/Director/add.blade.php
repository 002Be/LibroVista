...
@section("title", "Yönetmen Ekle")
@section("content")

<div class="card" style="background-color:dimgray;">
    <div class="card-header">
        <h5 class="card-title mb-0 text-white">Eklenecek Yönetmen Hakkında</h5>
    </div>
    <div class="card-body text-white">
        <form method="POST" action="{{route('content.director.add')}}" enctype="multipart/form-data"> @csrf
            <input type="hidden" name="process" value="publicInfo">
                ...
                <div class="form-check">
                    <label class="form-check-label" for="flexCheckChecked">
                        Yönetmen ekleme koşullarını okudum kabul ediyorum.
                    </label>
                    <input class="form-check-input" type="checkbox" id="flexCheckChecked" required>
                </div><br>
            <button type="submit" class="btn btn-success w-100">Yönetmeni Ekle</button>
        </form>
    </div>
</div>
...
<div style="background-color:dimgray;" class="card" style="position: fixed; width:300px">
    <div class="card-header">
        <h5 class="card-title mb-0 text-center">Yönetmen Ekleme Kuralları</h5>
    </div>
